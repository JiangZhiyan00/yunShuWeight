 ...
 */
@Component
public class HuZhouWeightJourMonitor implements IBaseHandler {
    ...
    private static final String SCHEMA_CODE = "incontrol_hz";

    private static final String MONITOR_NAME = "云枢_湖州管理中心超重车(167:station_info:weight_jour)";

    /**
     * redis的key
     */
    private static final String YUNSHU_OVERWEIGHT_MONITOR_KEY = "YUNSHU:OVERWEIGHT:MONITOR:HUZHOU";

    @Override
    @XxlJob(value = "huZhouWeightJourMonitor")
    public void doHandle() {
        ...
        try {
            ...
            XxlJobHelper.log("上次监控云枢湖州管理中心超重车数据条数:" + existNum + "条");
            ...
            XxlJobHelper.log("此时云枢湖州管理中心超重车数据条数:" + allFormData.size() + "条");
            if (existNum > allFormData.size()) {
                monitorResult = MonitorResult.fail(MONITOR_NAME,"此时云枢湖州管理中心超重车数据条数:"
                        + allFormData.size() + ",小于上次监控云枢湖州管理中心超重车数据条数:" + existNum);
            }
